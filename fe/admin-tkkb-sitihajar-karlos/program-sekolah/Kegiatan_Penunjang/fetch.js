...
export function postKegiatanPenunjang({ kegiatan_penunjang }) {
  ...
  return fetch(`${import.meta.env.VITE_API_KEY}/programsekolah/kegiatanpenunjang`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error(`Gagal menambahkan data kegiatan penunjang. Status: ${response.status}`);
      }
      return response.json();
    })
    .then(result => {
...
export function updateKegiatanPenunjangById(id, { kegiatan_penunjang }) {
  ...
  return fetch(`${import.meta.env.VITE_API_KEY}/programsekolah/kegiatanpenunjang/${id}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error(`Gagal memperbarui data kegiatan penunjang. Status: ${response.status}`);
      }
      return response.json();
    })
    .then(result => {
...
export function deleteKegiatanPenunjangById(id) {
  ...
  .then(response => {
    if (!response.ok) throw new Error(`Gagal menghapus data kegiatan penunjang. Status: ${response.status}`);
    return response.json();
  })
  .catch(error => {
