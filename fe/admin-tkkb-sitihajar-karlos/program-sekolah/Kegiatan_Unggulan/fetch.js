...
export function postKegiatanUnggulan({ nama_kegiatan, deskripsi_kegiatan }, imageFile) {
  ...
  return fetch(
		`${import.meta.env.VITE_API_KEY}/programsekolah/kegiatanunggulan`,
		{
			method: "POST",
			headers: {
		...
		.then((response) => {
			if (!response.ok) {
				throw new Error(
					`Gagal menambahkan data kegiatan unggulan. Status: ${response.status}`
				);
			}
			return response.json();
		})
...
export function updateKegiatanUnggulanById(id, { nama_kegiatan, deskripsi_kegiatan }, imageFile) {
  ...
  return fetch(
		`${import.meta.env.VITE_API_KEY}/programsekolah/kegiatanunggulan/${id}`,
		{
			method: "POST",
			headers: {
				Authorization: `Bearer ${localStorage.getItem("token")}`,
		...
		.then((response) => {
			if (!response.ok) {
				throw new Error(
					`Gagal memperbarui data kegiatan unggulan. Status: ${response.status}`
				);
			}
			return response.json();
		})
...
export function deleteKegiatanUnggulanById(id) {
  return fetch(
		...
		.then((response) => {
			if (!response.ok)
				throw new Error(
					`Gagal menghapus data kegiatan unggulan. Status: ${response.status}`
				);
			return response.json();
		})
		.catch((error) => {
