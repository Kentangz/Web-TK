...
export function postOutput({ output_description }) {
  ...
  return fetch(`${import.meta.env.VITE_API_KEY}/programsekolah/output`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error(`Gagal menambahkan data output yang dihasilkan. Status: ${response.status}`);
      }
      return response.json();
    })
    .then(result => {
...
export function updateOutputById(id, { output_description }) {
  ...
  return fetch(`${import.meta.env.VITE_API_KEY}/programsekolah/output/${id}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error(`Gagal memperbarui data output yang dihasilkan. Status: ${response.status}`);
      }
      return response.json();
    })
    .then(result => {
...
export function deleteOutputById(id) {
  ...
  .then(response => {
    if (!response.ok) throw new Error(`Gagal menghapus data output yang dihasilkan. Status: ${response.status}`);
    return response.json();
  })
  .catch(error => {
