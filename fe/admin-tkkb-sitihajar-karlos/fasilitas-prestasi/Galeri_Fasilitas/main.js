...
import { getAllGambarFasilitas, postGambarFasilitas, updateGambarFasilitasById, deleteGambarFasilitasById } from './fetch.js';
  ...
  <div class="main-content">
    ...
    <div class="gallery-card">
      <div class="card-header">
        <div class="card-title">Data Gambar Fasilitas</div>
      </div>
      <div class="card-body">
...
function attachButtonHandlers(row) {
  ...
  btnHapus.addEventListener('click', () => {
    deleteGambarFasilitasById(id)
      .then(() => {
        row.remove();
        showToast("Data gambar fasilitas berhasil dihapus.", "error");
      })
      .catch(() => {
        showToast("Gagal menghapus data gambar fasilitas.", "error");
      });
  });
  ...
  btnEdit.addEventListener('click', () => {
    ...
    btnUpload.addEventListener('click', () => {
      ...
      fileInput.onchange = () => {
        ...
        reader.onload = e => {
          ...
          img.src = newSrc;
          showToast("Foto fasilitas berhasil di upload.", "success");
        };
    ...
    btnSimpan.addEventListener('click', () => {
      updateGambarFasilitasById(id, { imageFile: selectedFile })
        .then(updated => {
          if (updated) {
            originalSrc = updated.img;
            img.src = updated.img;
            showToast("Data gambar fasilitas berhasil disimpan.", "success");
          }
          tdOptions.innerHTML = `
            <button class="btn-edit">Edit</button>
            <button class="btn-hapus">Hapus</button>
          ...
          .catch(() => {
            showToast("Gagal menyimpan data gambar fasilitas.", "error");
          });
      });
...
document.querySelector('.btn-tambah').addEventListener('click', () => {
  ...
  fileInput.onchange = () => {
    ...
    postGambarFasilitas({ imageFile: file })
      .then(result => {
        if (result) {
          tambahBaris(result);
           showToast("Foto fasilitas berhasil ditambahkan.", "success");
        }
        enableAllButtons();
        reenabled = true;
      })
        .catch(() => {
    showToast("Gagal menambahkan foto fasilitas.", "error");
    enableAllButtons();
    reenabled = true;
  });
  };
...
getAllGambarFasilitas()
  ...
  .catch(() => {
    showToast("Gagal memuat data gambar fasilitas.", "error");
  });
