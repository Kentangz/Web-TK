    ...
    return gambarList.map(gambar => `
      <div class="gallery-item">
        <div class="image-wrapper">
          <img src="${gambar.image}" class="gallery-img" alt="Galeri Kegiatan"
              onerror="this.src='/images/fallback.jpg';" />
          <div class="gallery-caption programsekolah-text">${gambar.nama_kegiatan || ''}</div>
        </div>
      </div>
    `).join('');
  } catch (error) {
...
export async function fetchGambarGaleriKegiatanPreview(limit) {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_KEY}/programsekolah/gallerykegiatan`);
    if (!response.ok) throw new Error(`Error! Status: ${response.status}`);
